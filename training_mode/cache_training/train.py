...
def experiment(conf, model, criterion, ft_criterion, train_data_loader, test_data_loader):
    ...
    backbone.config_cache(exits=cache_exits, hits=cache_hits)
    ...
    if conf.train_epochs > 0:
        backbone.config_cache(vectors=True)
    ...
    backbone.config_cache(vectors=False)
    model = model.to(conf.test_device)
    model.eval()
    model.backbone.eval()
    for ex in cache_exits:
        ex.eval()
    exits_df = pd.DataFrame(columns=["Confidence", "ExitNumber", "HitTime",
    ...
    test_confidences = [.0]  # [i/100 for i in range(0, 101, 2)]
    ...
    with torch.no_grad():
        if conf.distillation_test:
            ...
            for confidence in test_confidences:
                ...
                ems = [ExitMeter(i, mm) for i in range(num_exits + 1)]
                ...
                for i in range(num_exits + 1):
                    print(ems[i])
                # print(
                #     f"************RESULTS FOR CONFIDENCE: {confidence}************")
                # print(samplewise_hit_times, hit_counts, sum(hit_counts), total)
                # print(
                #     f'Non-cached model accuracy: {100 * nc_correct / total:.2f} %, out of: {total}, time:{nc_total_time}')
                # print(
                #     f'Cached model accuracy: {100 * sum(correct) / total:.2f} %, out of: {total} time: {total_time}')
                ...
                #     "CacheAccuracy": -1 if sum([m.hit_count for m in ems][:-1]) == 0 else round(100 * sum([m.cached_correct for m in ems][:-1]) / sum([m.hit_counts for m in ems][:-1]), 2),
                #     "CachedAccuracy": round(100 * sum([m.correct for m in ems]) / total, 2),
                ...
                # for i in range(num_exits+1):
                #     print(
                #         f'EXIT {i} | hit times {100 * hit_times[i] / nc_total_time:.2f} % , out of {total_time:.2f} sec, nc time {nc_total_time:.2f}')
...
def place_experiment(conf):
    ...
    test_data = PlaceDataset(
        conf.data_root, conf.test_file, conf.classes_file)
    ...
    experiment(conf, model, criterion, ft_criterion,
               train_data_loader, test_data_loader)


def face_experiment(conf):
    ...
    test_data = ImageDataset(
        conf.data_root, conf.test_file, classes_file=conf.classes_file, name_as_label=True)
    test_data_loader = DataLoader(test_data,
                                  conf.batch_size, True, num_workers=0)
    ...
    experiment(conf, model, criterion,
               ft_criterion, train_data_loader, test_data_loader)
...
if __name__ == '__main__':
    ...
    if args.experiment == "Face":
        face_experiment(args)
    elif args.experiment == "Place":
        place_experiment(args)
    else:
        logger.info("Unrecognized experiment name!")
